...
#For AEG example with 265 applicants,
#it takes about 5 min to run the prog in Python3 shell.
...
mypath="/home/david/PYenv/Resume_scr/NLP_Resume/EAEapp" #enter your path here where you saved the resumes
...
def create_profile(file):
    ...
    keyword_dict = pd.read_csv('/home/david/PYenv/Resume_scr/NLP_Resume/template/template.csv', encoding='ISO-8859-1')
    ...
    Poor_words = [nlp(text) for text in keyword_dict['Poor Perf'].dropna(axis = 0)]
    Avg_words = [nlp(text) for text in keyword_dict['Avg Perf'].dropna(axis = 0)]
    AAvg_words = [nlp(text) for text in keyword_dict['Above Avg Perf'].dropna(axis = 0)]
    VG_words = [nlp(text) for text in keyword_dict['V.G Perf'].dropna(axis = 0)]
    ...
    matcher.add('Poor', None, *Poor_words)
    matcher.add('Avg', None, *Avg_words)
    matcher.add('Above_Avg', None, *AAvg_words)
    matcher.add('Very_Good', None, *VG_words)
    doc = nlp(text)
...
sample2=new_data.to_csv('/home/david/PYenv/Resume_scr/NLP_Resume/template/EAEselect.csv')
